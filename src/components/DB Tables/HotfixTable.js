...
function HotfixTable() {
    const[hotfixAtr,setHotfixAtr]=useState([])

    useEffect( ()=> {
        fetch("http://localhost:8080/hotfix/getAll")
        .then(res=>res.json())
        .then((result)=>{
            setHotfixAtr(result);
        })
    ...
    const columns = [
        ...
        { field: 'hotFixId', headerName: 'Hotfix ID', width: 200 },
        { field: 'source', headerName: 'Source', width: 200 },
        { field: 'description', headerName: 'Description', width: 200 },
        { field: 'installedOn', headerName: 'Installed On', width: 200 },
        { field: 'installedBy', headerName: 'Installed By', width: 200 },
        { field: 'biosSeralNumber', headerName: 'Bios Serial Number', width: 200 },
    ]

    const rows = hotfixAtr.map( vals => (
          {id: vals.id , 
            hotFixId: vals.hotFixId,
            source: vals.source,
            description: vals.description,
            installedOn: vals.installedOn,
            installedBy: vals.installedBy,
        ...
        <div>
            ...
            <Grid container style={tableStyle}>
                <Grid item xs={16} sx={{textAlign:"center"}}>
                    <span style={headingStyle}><h3>Hotfix Table</h3></span>
                </Grid>
            </Grid>
